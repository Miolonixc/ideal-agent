...
- **HTTP** — `python3 main.py http --port 8080` (для Android-компаньона и вебхуков):
  ...
  контекст удаляется при следующем обращении к агенту после 12 часов
  бездействия (и всегда ограничен 32 сессиями);
...
В TUI при первом запуске недоверенного skill появится запрос с его capabilities;
подтверждение сохраняется в конфиг. HTTP, IDE и Telegram никогда не подтверждают
extensions автоматически.
