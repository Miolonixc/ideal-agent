...
class TestAndroidCompanion(unittest.TestCase):
    def test_version_is_installable_upgrade_and_visible_in_settings(self):
        ...
        self.assertIn("cancelAgentStream(host, accessToken, sessionId)", activity)
        self.assertIn("clearAgentSession(host, accessToken, sessionId)", activity)
        self.assertIn("/sessions/${java.net.URLEncoder.encode(sessionId, \"UTF-8\")}/cancel", activity)
        self.assertIn('conn.requestMethod = "DELETE"', activity)
