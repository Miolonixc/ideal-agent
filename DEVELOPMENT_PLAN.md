...
## Аудит корректности ядра — перед новым функционалом

1. Сделать сжатие истории отказоустойчивым: сбой отдельного summarization
   запроса не должен превращать уже полученный ответ агента в ошибку. —
   выполнено, покрыто регресс-тестом.
2. Проверить изоляцию состояния сессий и конкурентный доступ к SQLite/audit;
   зафиксировать безопасную модель исполнения регресс-тестами. — выполнено:
   агент сериализует выполнение сессий, audit-log получил собственную блокировку,
   добавлены параллельные тесты.
3. Проверить отмену и очистку streaming при ошибках/закрытии агента, чтобы не
   оставались зависшие сессии или временные вложения. — выполнено: закрытие
   ожидает завершения отменённого потока и не допускает новые запросы.
4. Проверить полноту SSE у OpenAI-совместимых провайдеров. — выполнено:
   оборванный или полностью повреждённый поток теперь является ошибкой.
5. После закрытия инвариантов — выбирать следующий функциональный модуль.

## Следующий функциональный модуль — явная локальная память

1. Добавить opt-in команды для сохранения и поиска пользовательских фактов без
   автоматической отправки их в LLM. — выполнено: `/remember` и `/memories`.
